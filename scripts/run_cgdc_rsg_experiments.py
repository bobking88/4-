...
CGDC_COMPLETE_FLAGS = (*RSG_COMPLETE_FLAGS, "--enable-cgdc")
CONFIGURATION_FLAGS = {
    "rsg_complete": RSG_COMPLETE_FLAGS,
    "cgdc_complete": CGDC_COMPLETE_FLAGS,
    "cgdc_shared_features": (*CGDC_COMPLETE_FLAGS, "--cgdc-shared-features"),
    "cgdc_unconditional": (*CGDC_COMPLETE_FLAGS, "--cgdc-unconditional"),
    "cgdc_no_decomposition_loss": (*CGDC_COMPLETE_FLAGS, "--lambda-decomposition", "0"),
}
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the registered CGDC-RSG-HRGV experiment matrix.")
    parser.add_argument("--manifest", type=Path, required=True)
    parser.add_argument("--dataset-root", type=Path, required=True)
    parser.add_argument("--output-root", type=Path, default=Path(__file__).resolve().parents[1] / "outputs" / "training")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    commands = build_experiment_commands(
        ...
        args.python_executable, args.device, args.torch_home, args.stage,
    )
