...
from hrgv_network import HierarchicalRiskGatedVerificationNet, regret_gate_targets, weighted_soft_gate_loss
...
def main():
    ...
    save_json(OUT / 'registered_protocol.json', {
        'scope': 'exploratory validation-only; pre-verifier fusion; one fixed expert checkpoint',
        'expert_seed': 20260728, 'gate_seeds': [20260912, 20260913, 20260914],
        'objectives': ['equal', 'fusion_nll', 'regret_bce'],
        'epochs': 30, 'batch_size': 256, 'optimizer': 'AdamW', 'lr': .001,
        'weight_decay': .0001, 'selection': 'lowest validation fusion NLL; earliest tie',
    ...
    for seed in (20260912, 20260913, 20260914):
        ...
        for objective in ('fusion_nll', 'regret_bce'):
            torch.manual_seed(seed)
            ...
            for epoch in range(30):
                ...
                if record['fusion_nll_nats'] < best_metric:
                    best_metric, best_state = record['fusion_nll_nats'], copy.deepcopy(gate_net.state_dict())
                if (epoch+1) % 10 == 0:
                    print(seed, objective, record, flush=True)
            gate_net.load_state_dict(best_state)
            run_dir = OUT / f'{objective}_seed{seed}'
            run_dir.mkdir()
            torch.save(best_state, run_dir/'gate_state.pt')
            save_json(run_dir/'history.json', history)
            with torch.no_grad():
                gates = torch.sigmoid(gate_net(val['features']))
                selected = min(history, key=lambda r: r['fusion_nll_nats'])
