...
def build_report():
    ...
    add_table(doc, ["设置", "Macro F1（%）", "目标代理 F1（%）", "目标漏选率（%）", "含钛干扰误入目标（%）", "金属光泽干扰误入目标（%）"], [
    ...
    add_table(doc, ["序号", "创新点", "具体体现"], [
        ...
        ("5", "矿物种类—选矿角色分层一致性原型", "利用已保留的 17 类种类标签与四类角色映射，在共享视觉表征上联合种类、角色和目标代理任务；并通过两项三随机种子组件消融，报告其总体分数与关键风险指标的可量化取舍。"),
    ], [1.0, 5.0, 9.8], 10)
    ...
    add_body(doc, "第一，优先补充经过审核的钛磁铁矿样本，或采集少量由专家/化验确认的真实钒钛矿颗粒图像，用于建立外部测试集。第二，回填可获得的产地、摄影者、收藏机构或页面来源信息，构建按来源留出的严格泛化评价。第三，在已完成的一致性项和困难负样本项消融基础上，继续考察种类交叉熵、目标代理辅助头的影响，并在目标召回率达到预设下限时最小化含钛、金属光泽干扰误入目标率。第四，建立独立未知矿物图像集，开展最大 Softmax 概率、能量分数等开放集拒识评价。第五，结合 XRF、磁性或化验信息，形成 RGB 初筛与成分确认相结合的多模态工作流。")
