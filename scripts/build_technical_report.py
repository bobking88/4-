...
def build_report():
    ...
    add_heading(doc, "4.5 矿物种类—选矿角色分层一致性模型", 2)
    add_body(doc, "在现有四分类角色标签之外，数据清单还保留了 17 类细粒度 mineral_label。基于这一已有事实标签，构建共享 EfficientNet-B0 主干的分层模型：一条分支输出四类选矿角色，另一条分支输出 17 类矿物种类，同时设置目标代理/非目标代理辅助头。将种类概率按预先定义的“种类—角色”映射聚合为角色概率，并以 Kullback-Leibler 散度约束其与角色头输出保持一致。训练总损失由角色加权交叉熵、种类交叉熵、目标代理二分类损失、分层一致性损失和困难负样本表示分离项构成。该模型研究的是公开图像标签层级能否改善视觉表征，不将其解释为矿物化学成分或实际分选动作预测。")
    add_figure(doc, FIG_DIR / "fig8_hierarchical_architecture.png", 15.4, "图 2 矿物种类—选矿角色分层一致性模型结构")
    ...
    add_body(doc, "分层模型在 17 类矿物种类头上的平均测试 Accuracy 为 56.80%±2.23%，而角色主任务的 Macro F1 为 73.41%±2.40%。这符合该数据的任务特点：网页主题矿物、伴生矿物与基质可能共存，细粒度种类标签更难由单张宏观图片稳定确定；但种类概率聚合后的角色信息仍可为粗粒度识别提供辅助。相较普通 EfficientNet-B0，分层模型把目标漏选率降低 5.58 个百分点、目标代理 F1 提高 1.59 个百分点，却使含钛干扰和金属光泽干扰误入目标的比例分别增加 1.09 和 2.84 个百分点。因此，本阶段结果只能支持“分层监督能改善目标类召回、但尚未满足困难负样本风险控制”的结论。下一步应进行种类损失、层级一致性损失、二分类辅助损失和困难负样本约束的组件消融，并在目标召回约束下优化困难干扰误入率。")
    ...
    add_table(doc, ["序号", "创新点", "具体体现"], [
        ...
        ("5", "矿物种类—选矿角色分层一致性原型", "利用已保留的 17 类种类标签与四类角色映射，在共享视觉表征上联合种类、角色和目标代理任务；结果揭示了目标召回改善与困难负样本风险上升的可量化取舍。"),
    ], [1.0, 5.0, 9.8], 10)
    ...
    add_table(doc, ["问题", "对结果解释的影响", "处理原则"], [
        ...
        ("改进模型的总体收益有限", "角色感知和分层模型的均值提升均小于或接近种子间波动；分层模型还提高困难干扰误入目标率", "报告取舍而非宣称稳定领先，补做组件消融与受约束风险优化"),
    ], [3.3, 6.0, 6.5], 9.5)
    add_heading(doc, "7.2 后续研究计划", 2)
    add_body(doc, "第一，优先补充经过审核的钛磁铁矿样本，或采集少量由专家/化验确认的真实钒钛矿颗粒图像，用于建立外部测试集。第二，回填可获得的产地、摄影者、收藏机构或页面来源信息，构建按来源留出的严格泛化评价。第三，围绕分层模型开展组件消融：分别移除种类交叉熵、层级一致性、目标代理辅助头和困难负样本约束，并在目标召回率达到预设下限时最小化含钛、金属光泽干扰误入目标率。第四，建立独立未知矿物图像集，开展最大 Softmax 概率、能量分数等开放集拒识评价。第五，结合 XRF、磁性或化验信息，形成 RGB 初筛与成分确认相结合的多模态工作流。")
    ...
    conclusions = [
        ...
        "矿物种类—选矿角色分层一致性模型将种类概率与角色概率联合约束，目标类召回提高，但困难干扰误入目标率也上升；当前应将其视为可量化的召回—风险取舍原型，而非已被证明的稳定性能改进。",
