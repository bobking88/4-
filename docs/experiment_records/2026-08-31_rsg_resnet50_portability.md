...
该设计对应“主干替换不变性命题”：B.1--B.3 的数学前提仅要求两位专家输出角色概率、门控为凸权重以及后悔分支使用 stop-gradient，故其形式性质与具体主干无关。ResNet50 实验检验的是该主干无关形式在实际训练中是否仍带来较低的经验路由后悔；它不能将形式命题误写成任意网络上的性能保证。
