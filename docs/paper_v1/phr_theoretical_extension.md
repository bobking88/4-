...
日期：2026-09-05 至 2026-09-06。状态：解析推导与数值核验完成；八项预注册验证集筛选完成但完整 PHR 未晋级至正式多随机种子测试。
...
3. 真实验证集筛选：修正后的 RSG 对照与 7 个 PHR 配置已在固定种子 `20260728` 完成，未使用测试集选配置。
4. 筛选结果：完整 PHR 的两条平均成对遗憾由 `0.4187/0.6374` 降至 `0.1813/0.2350`，但 Macro F1 由 `0.7485` 降至 `0.7414`、目标类召回由 `0.7360` 降至 `0.6880`，且两类关键误入率均上升。因此不满足任一预注册晋级条件；不启动正式多种子实验。
5. 该结果为 P6--P8 所述全局耦合边界提供真实数据上的机制性佐证：局部边际遗憾下降不推出全局四分类风险改善。它不是 PHR 的普遍性能优势证据。
6. 将 P3/P5 的预测量在冻结真实输出上复算，及在独立来源划分上重新设计全局风险目标，可作为后续工作；理论上界即使成立但过松，也必须如实报告。
7. 正式报告只可将本节列为“候选网络理论、实现验证与负结果边界”；不得写“PHR 显著优于 RSG”。
